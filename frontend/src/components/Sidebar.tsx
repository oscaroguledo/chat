...
export function Sidebar({
  ...
  // Contacts view
  const [contactFilter, setContactFilter] = useState<'all' | 'blocked'>('all');
  const blockedContactIds: string[] = []; // TODO: Add blocked contacts to mockData
  const filteredContacts = contactFilter === 'blocked'
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredContacts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-chat-muted dark:text-chat-muted">
            <p className="text-sm">No {contactFilter === 'blocked' ? 'blocked contacts' : 'contacts'}</p>
          </div>
  ...
  const renderCalls = () => {
    ...
    const filteredCalls = callFilter === 'missed' 
      ? mockCalls.filter(c => c.type === 'missed')
      : mockCalls;
        ...
        <div className="flex-1 overflow-y-auto">
          {filteredCalls.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-chat-muted dark:text-chat-muted">
              ...
              <p className="text-sm">No {callFilter === 'missed' ? 'missed calls' : 'calls'}</p>
            </div>
